...
const SkillSection = ({ sortedSkillsData, handleExpand, cardClass }: SkillSectionProps) => (
  (
    <section className="fadeIn mt-6 grid w-full grid-cols-1 gap-6 pt-2 md:grid-cols-2 lg:grid-cols-4">
      {sortedSkillsData.map((category) => (
        <article
          key={category.title}
          id="skill_cards"
          className={`w-full rounded-xl border border-primary_light bg-primary_light/20 p-4 shadow shadow-primary_dark ${cardClass}`}
        >
